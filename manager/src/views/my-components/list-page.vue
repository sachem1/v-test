<template>
    <div>
        <smart-form ref="mainForm" v-model="editingModel" :isInline="true" :validateRules="validateRules" :operationMode="operationMode" :entityName="entityName" :serviceName="serviceName" :enums="commonEnums" :formItems="formItems" @on-model-change="onMainFormSaved"></smart-form>
    </div>
</template>
<script>
import Vue from 'vue';
    ...
    methods: {
        async init () {
            ...
            var metadata = await vm.$store.dispatch({
                        ...
                        data: {metaFileName: this.metaFileNameForBind, query : this.$route.query}
                    });
