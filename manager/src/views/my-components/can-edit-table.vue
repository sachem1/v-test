<template>
    <div>
        <div style="position: relative; height: 100%;" v-if="!isInline">
            <paged-table ref="testTable" :enums="commonEnums" :serviceName="serviceName" :listUrl="listUrl" :searchModel="searchModel" :searchItems="searchItems" :columns="columns" :bus="bus" :addBehaviorSetting="addBehaviorSetting" :title="tableTitle" :disableImportExport="disableImportExport" :disableAdd="disableAdd" :disableBatchDelete="disableBatchDelete" @on-request-inline-page="handleInlinePageRequest"></paged-table>
            <smart-form ref="mainForm" v-model="editingModel" :autoClose="autoClose" :visible="showModalForm" :validateRules="validateRules" :operationMode="operationMode" :entityName="entityName" :serviceName="serviceName" :enums="commonEnums" :formItems="formItems" @on-visible-change="onMainFormVisibleChanged" @on-model-change="onMainFormSaved"></smart-form>
        </div>
        <div style="position: relative; height: 100%;" v-if="isInline">
            <smart-form ref="mainForm" v-model="editingModel" :autoClose="autoClose" :visible="showModalForm" :isInline="true" :validateRules="validateRules" :operationMode="operationMode" :entityName="entityName" :serviceName="serviceName" :enums="commonEnums" :formItems="formItems" @on-visible-change="onMainFormVisibleChanged" @on-model-change="onMainFormSaved"></smart-form>
            <paged-table ref="testTable" :enums="commonEnums" :serviceName="serviceName" :listUrl="listUrl" :searchModel="searchModel" :searchItems="searchItems" :columns="columns" :pageSize="pageSize" :bus="bus" :title="tableTitle" :disableImportExport="disableImportExport" :disableAdd="disableAdd" :disableBatchDelete="disableBatchDelete" :addBehaviorSetting="addBehaviorSetting" @on-request-inline-page="handleInlinePageRequest"></paged-table>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        buildUiConfig(vm, config) {
            ...
            if (config.hasOwnProperty('listUrl'))
            ...
            if (config.searchModel)
                vm.searchModel = config.searchModel;
            else
                vm.searchModel = {};
