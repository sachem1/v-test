      ...
      <Form ref="mainForm" :model="editForm" label-position="left" :label-width="135" :rules="validateRules">
        <Row>
          ...
          <FormItem label="账册号" label-position="right" prop="emsNo" :show-message="false">
            <ems-No-List ref="currentEmsNoSelect" :class="ivuRequired" :emsNo="editForm.emsNo" @on-select-change="changeEmsNo"></ems-No-List>
          </FormItem>
          </Col>
          <Col :sm="24" :md="24" :lg="12">
        ...
        <Row>
          <Col :sm="24" :md="24" :lg="12">
          <FormItem label="企业料号" label-position="right" prop="erpSku" :label-colon="true" :show-message="false">
            <Input v-model="editForm.erpSku" :disabled="operationMode === 'edit'" :class="ivuRequired"></Input>
          </FormItem>
          </Col>
  ...
  export default {
    ...
    methods: {
      ...
      messageConfirm() {
        ...
        this.opreateOnlineData("goodsErp/modifyGoodsErp");
        this.messageparams.displayMessage = false;
      },
      existDataOnline(data) {
        ...
        var cdata = vm.$store
          .dispatch({
            ...
            if (res.data.data.length > 0) {
              vm.editForm.id = res.data.data[0].id
              vm.existGoodsErp = true;
              vm.messageparams.displayMessage = true;
              vm.messageparams.messageArr = [
                { code: 1, message: "企业料号" + vm.editForm.erpSku + "已经存在,是否覆盖" }
              ];
            }
            else {
              vm.opreateOnlineData("goodsErp/addGoodsErp");
            }
          })
          .catch(error => {
            console.log(error);
