...
<template>
	<div class="main"
		...
		<Spinner name="fading-circle"
						 color="#2d8cf0"
						 v-show="isSpinShowing"
						 :noFadeIn="noFadeIn" />
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			cancelSpin () {
				if (this.spinTimeout != null && new Date().getTime() - this.spinQueuedTime.getTime() < spinDelay) {
			...
			async handleClickUserDropdown (name) {
				if (name === 'ownSpace') {
					...
					await this.$store.dispatch({
						type: 'user/logout',
						data: { 'token': util.getToken(), 'account': util.getCookieValue('account') }
					});
